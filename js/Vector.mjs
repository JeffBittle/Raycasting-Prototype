 ...
 * 13th October 2018: (Jeff Bittle Edits)
   * Fixed limitTo() to prevent raising values lower than the limit up to it
   * Fixed minComponent() and maxComponent() to return the correct value regardless of negative numbers
   * Added a function to return a new random 2D unitVector
   * Removed a couple of redundant comments
   * Replaced a few still existing 'var' declarations with more appropriate 'let' and 'const' ES6 declaractions
 */
...
class Vector {
  ...
  random2D() {
    return new Vector((Math.random() * 2) - 1, (Math.random() * 2) - 1).unitVector;
  }
	 ...
	 */
	get minComponent() {
		return Math.min(Math.abs(this.x), Math.abs(this.y)) === Math.abs(this.x) ? this.x : this.y;
	}
	 ...
	 */
	get maxComponent() {
		return Math.max(Math.abs(this.x), Math.abs(this.y)) === Math.abs(this.x) ? this.x : this.y;
	}
}
